...
add_action( 'admin_init', 'cf7ru_check_parent_plugin' );
function cf7ru_check_parent_plugin() {
    if ( is_admin() && current_user_can( 'activate_plugins' ) &&  !is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) ) {
        add_action( 'admin_notices', 'cf7ru_parent_not_active' );
...
function cf7ru_parent_not_active(){
    ?>
	<div class="error">
